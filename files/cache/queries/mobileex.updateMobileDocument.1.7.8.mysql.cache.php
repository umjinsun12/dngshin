...
$query = new Query();
$query->setQueryId("updateMobileDocument");
...
${'is_mobile27_argument'} = new Argument('is_mobile', $args->{'is_mobile'});
${'is_mobile27_argument'}->ensureDefaultValue('N');
if(!${'is_mobile27_argument'}->isValid()) return ${'is_mobile27_argument'}->getErrorMessage();
if(${'is_mobile27_argument'} !== null) ${'is_mobile27_argument'}->setColumnType('char');
...
,new UpdateExpression('`is_mobile`', ${'is_mobile27_argument'})
));
$query->setTables(array(
new Table('`dbigrus_mobileex_documents`', '`mobileex_documents`')
));
