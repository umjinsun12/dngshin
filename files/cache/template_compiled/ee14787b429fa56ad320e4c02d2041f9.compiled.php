...
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/sketchbook5_youtube','__setting.html') ?>
<!--#Meta:modules/board/skins/sketchbook5_youtube/js/board.js--><?php $__tmp=array('modules/board/skins/sketchbook5_youtube/js/board.js','body','','');Context::loadFile($__tmp);unset($__tmp); ?>
<?php if($__Context->mi->link_board){ ?><!--#Meta:modules/board/skins/sketchbook5_youtube/js/link.js--><?php $__tmp=array('modules/board/skins/sketchbook5_youtube/js/link.js','body','','');Context::loadFile($__tmp);unset($__tmp);
} ?>
<?php if($__Context->mi->default_style=='viewer' && $__Context->grant->view){;
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/sketchbook5_youtube','_viewer.html');
} ?>
...
<div id="bd" class="bd <?php echo $__Context->_COOKIE['use_np'] ?> <?php echo $__Context->mi->fdb_count;
...
if($__Context->mi->select_lst_more){ ?> select_lst_cloud<?php } ?>">
<div class="bd_hd clear">
	<?php if(!$__Context->mi->breadcrumb){;
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/sketchbook5_youtube','_breadcrumb.html');
} ?>
	
	<?php if($__Context->mi->font_btn!='N'){ ?><div class="bd_font fr"<?php if($__Context->oDocument->isExists() || $__Context->mi->font_btn=='2'){ ?> style="display:none"<?php } ?>>
		<a class="select tg_btn2" href="#bd_font"><b>T</b><strong><?php if($__Context->mi->font=='ng'){ ?>나눔고딕<?php }elseif($__Context->mi->font=='window_font'){;
...
echo $__Context->lang->best_font;
		...
		<div id="bd_font" class="tg_cnt2"><button type="button" class="tg_blur2"></button>
			<ul>
				...
				<li class="ng<?php if($__Context->mi->font=='ng'){ ?> on<?php } ?>"><a href="#popup_menu_area">나눔고딕</a><em>✔</em></li>
	...
	<div class="bd_set fr m_btn_wrp">
		...
		<a id="viewer_with" class="viewer_with bubble<?php if($__Context->mi->viewer_with=='2' || $__Context->_COOKIE['cookie_viewer_with']=='Y'){ ?> on<?php } ?>" href="#" title="<?php echo $__Context->lang->with_viewer_info ?>."><em>✔</em> <strong><?php echo $__Context->lang->link_site_viewer ?></strong></a>
		<?php }else{ ?>
		<?php if(!$__Context->oDocument->isExists() && $__Context->mi->viewer_with!='N'){ ?><a id="viewer_with" class="viewer_with rd_viewer bubble<?php if($__Context->mi->viewer_with=='2' || $__Context->_COOKIE['cookie_viewer_with']=='Y'){ ?> on<?php } ?>" href="#" title="<?php echo $__Context->lang->with_viewer_info ?>."><em>✔</em> <strong><?php echo $__Context->lang->with_viewer ?></strong></a><?php } ?>
		<?php } ?>
		<?php } ?>
		...
		<?php if($__Context->mi->font_btn=='2'){ ?><span class="font_select"><a class="select tg_btn2" href="#bd_font"><b class="tx_ico_chk">T</b><?php echo $__Context->lang->font ?><i class="arrow down"></i></a></span><?php } ?>
		...
		<a href="<?php echo getUrl('','module','document','act','dispDocumentManageDocument') ?>" onclick="popopen(this.href,'manageDocument');return false"><b class="tx_ico_chk">✔</b><?php echo $__Context->lang->cmd_manage_document ?></a>
...
<?php if($__Context->oDocument->isExists()){;
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/sketchbook5_youtube','_read.html');
} ?>
<?php if(!$__Context->oDocument->isExists() || ($__Context->oDocument->isExists() && !$__Context->mi->rd_lst)){ ?><div id="bd_lst_wrp">
<div class="tl_srch clear">
	
	<?php if($__Context->mi->title){ ?><div class="bd_tl ngeb">
		<h1 class="clear"><i class="bg_color"></i><a href="<?php echo getUrl('','mid',$__Context->mid) ?>"><?php echo $__Context->mi->title ?></a></h1>
	...
	<?php if($__Context->mi->srch_window!='N' && !Mobile::isMobileCheckByAgent()){ ?><div id="faq_srch"<?php if($__Context->mi->srch_window==3){ ?> class="clear"<?php };
if($__Context->mi->srch_window==2 || $__Context->search_keyword){ ?> style="display:block"<?php } ?>>
		<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/sketchbook5_youtube','_search.html') ?>
	</div><?php } ?>
</div>
...
<?php if($__Context->category_list&&count($__Context->category_list))foreach($__Context->category_list as $__Context->key=>$__Context->val){ ?>
	...
	<?php } ?>
<?php } ?>
	<?php if(!$__Context->mi->cnb){ ?><div id="bdCnb" class="bd_cnb clear css3pie<?php if($__Context->mi->cnb_open){ ?> open<?php } ?>">
		...
		<ul class="bubble bg_f_f9 css3pie">
			<li id="cnbMore"><a href="#" class="bubble" title="<?php echo $__Context->lang->category ?> <?php echo $__Context->lang->more ?>"><i class="ui-icon<?php if($__Context->mi->cnb_open){ ?> ui-icon-triangle-1-n<?php }else{ ?> ui-icon-triangle-1-s<?php } ?>"></i></a></li>
...
if($__Context->mi->cnb_count){ ?><small>(<?php echo $__Context->val->document_count ?>)</small><?php } ?></a>
			<?php if(count($__Context->val->children)){ ?><ul>
				<?php if($__Context->val->children&&count($__Context->val->children))foreach($__Context->val->children as $__Context->idx=>$__Context->item){ ?><li<?php if($__Context->category==$__Context->item->category_srl){ ?> class="on_"<?php } ?>><a href="<?php echo getUrl(category,$__Context->item->category_srl,'document_srl','','page','') ?>"<?php if($__Context->item->color!='transparent'){ ?> style="color:<?php echo $__Context->item->color ?>"<?php } ?>><?php echo $__Context->item->title ?></a></li><?php } ?>
			</ul><?php } ?>
		</li><?php } ?>
	</ul><?php } ?>
</div><?php } ?>
...
<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/sketchbook5_youtube','_list_normal.html') ?>
<?php }elseif($__Context->mi->default_style=='webzine'){ ?>
<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/sketchbook5_youtube','_list_webzine.html') ?>
<?php }elseif($__Context->mi->default_style=='gallery'){ ?>
<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/sketchbook5_youtube','_list_gallery.html') ?>
<?php }elseif($__Context->mi->default_style=='cloud_gall'){ ?>
<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/sketchbook5_youtube','_list_cloud_gall.html') ?>
<?php }elseif($__Context->mi->default_style=='faq'){ ?>
<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/sketchbook5_youtube','_list_faq.html') ?>
<?php }elseif($__Context->mi->default_style=='guest' && !$__Context->oDocument->isExists()){ ?>
<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/sketchbook5_youtube','_list_guest.html') ?>
...
<?php if(!$__Context->mi->display_setup_button){ ?><div class="btm_mn clear">
	...
	<?php if(Mobile::isMobileCheckByAgent()){ ?><div id="faq_srch" class="m_srch" style="display:block;float:none">
		<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/sketchbook5_youtube','_search.html') ?>
	</div><?php } ?>
	<div class="fl">
		<?php if(@in_array(home,$__Context->mi->btm_mn)){ ?><a class="btn_img" href="<?php echo getUrl('','mid',$__Context->mid,'page',$__Context->page,'document_srl','','listStyle',$__Context->listStyle) ?>"><i class="ico_16px list"></i> <?php echo $__Context->lang->cmd_list ?></a><?php } ?>
		
		<?php if($__Context->mi->srch_btm!='N' && !Mobile::isMobileCheckByAgent()){ ?><form action="<?php echo getUrl() ?>" method="get" onsubmit="return procFilter(this, search)" id="bd_srch_btm" <?php if($__Context->mi->srch_btm==2 || $__Context->search_keyword){ ?> class="on"<?php } ?>><input type="hidden" name="act" value="<?php echo $__Context->act ?>" />
			<input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
			<input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" />
			<input type="hidden" name="category" value="<?php echo $__Context->category ?>" />
			<span class="btn_img itx_wrp">
				<button type="submit" onclick="xGetElementById('bd_srch_btm').submit();return false" class="ico_16px search">Search</button>
				<label for="bd_srch_btm_itx"><?php echo $__Context->lang->cmd_search ?></label>
				<input type="text" name="search_keyword" id="bd_srch_btm_itx" class="srch_itx" value="<?php echo htmlspecialchars($__Context->search_keyword) ?>" />
			</span>
			<span class="btn_area">
	...
	<div class="fr">
		<?php if(@in_array(tag,$__Context->mi->btm_mn)){ ?><a class="btn_img m_no" href="<?php echo getUrl('act','dispBoardTagList') ?>"><i class="ico_16px tag"></i> <?php echo $__Context->lang->tag ?></a><?php } ?>
...
<form action="./" method="get" class="bd_pg clear"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" />
	...
	<?php if($__Context->page!=$__Context->prev_page){ ?><a href="<?php echo getUrl('page',$__Context->prev_page,'document_srl','','division',$__Context->division,'last_division',$__Context->last_division) ?>" class="direction">‹ Prev</a><?php } ?>
	<?php if($__Context->page==$__Context->prev_page){ ?><strong class="direction">‹ Prev</strong><?php } ?>
	...
	<?php if($__Context->page>($__Context->mi->page_count)/2+2){ ?><span class="bubble"><a href="#go_page" class="tg_btn2" title="<?php echo $__Context->lang->go_page ?>">...</a></span><?php } ?>
...
if($__Context->page_no!=1 && $__Context->page_no!=$__Context->page_navigation->last_page){ ?>
	...
	<?php }} ?>
	<?php if(($__Context->page+($__Context->mi->page_count+1)/2<$__Context->page_navigation->last_page) && ($__Context->mi->page_count+1<$__Context->page_navigation->last_page)){ ?><span class="bubble"><a href="#go_page" class="tg_btn2" title="<?php echo $__Context->lang->cmd_go_to_page ?>">...</a></span><?php } ?>
	...
	<?php if($__Context->page!=$__Context->next_page){ ?><a href="<?php echo getUrl('page',$__Context->next_page,'document_srl','','division',$__Context->division,'last_division',$__Context->last_division) ?>" class="direction">Next ›</a><?php } ?>
	<?php if($__Context->page==$__Context->next_page){ ?><strong class="direction">Next ›</strong><?php } ?>
	<div id="go_page" class="tg_cnt2 wrp">
		<button type="button" class="tg_blur2"></button>
		<input type="text" name="page" class="itx" /> / <?php echo $__Context->page_navigation->last_page ?> <button type="submit" class="btn">GO</button>
