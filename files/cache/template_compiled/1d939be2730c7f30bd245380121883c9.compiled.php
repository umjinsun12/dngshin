...
<?php if($__Context->mi->fdb_style=='fdb_v2' && $__Context->mi->default_style!='guest'){ ?>
<a class="nametag ui_font bubble" href="#" onclick="jQuery(this).nextAll('ul,.bd_pg').slideToggle();return false" title="Click! <?php echo $__Context->lang->comment ?> <?php echo $__Context->lang->cmd_view ?>~"><strong class="bg_color bx_shadow">Comment <b>'<?php echo $__Context->oDocument->getCommentcount() ?>'</b></strong></a>
<?php } ?>
...
<?php if($__Context->comment->get('depth')){ ?>
	<li id="comment_<?php echo $__Context->comment->comment_srl ?>" class="fdb_itm clear re bg<?php echo ($__Context->comment->get('depth'))%2 ?>" style="margin-left:<?php echo (($__Context->comment->get('depth')-1)%10+1)*2 ?>%">
		<i class="ico_16px re"></i><?php if($__Context->comment->get('depth')>10){ ?><i class="ico_16px re rere"></i><?php } ?>
<?php }else{ ?>
...
<?php } ?>
		...
		<div class="meta">
			...
			<?php if(!$__Context->comment->member_srl && $__Context->comment->homepage){ ?><a href="<?php echo $__Context->comment->homepage ?>" target="_blank"><?php echo $__Context->comment->getNickName() ?></a><?php } ?>
...
echo $__Context->file->download_url ?>" title="[File Size:<?php echo FileHandler::filesize($__Context->file->file_size) ?>/Download:<?php echo number_format($__Context->file->download_count) ?>]"><?php echo $__Context->file->source_filename ?></a></li><?php } ?>
		...
		<form action="./" method="get" onsubmit="return procFilter(this, input_password)"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
			...
			<span class="itx_wrp">
				...
				<input type="submit" value="<?php echo $__Context->lang->cmd_input ?>" class="btn" />
			</span>
		</form>
		<?php }else{ ?>
		...
		<div class="fdb_nav img_tx">
			...
			<a href="#" onclick="doCallModuleAction('comment','procCommentDeclare','<?php echo $__Context->comment->comment_srl ?>');return false">☎ <?php echo $__Context->lang->cmd_declare ?></a>
			<a href="#" onclick="doCallModuleAction('comment','procCommentVoteUp','<?php echo $__Context->comment->comment_srl ?>');return false"><i class="fdb_nav_ico"><em>♥</em></i> <?php echo $__Context->lang->cmd_vote ?></a>
			<?php if(!$__Context->mi->cmt_vote_down){ ?><a href="#" onclick="doCallModuleAction('comment','procCommentVoteDown','<?php echo $__Context->comment->comment_srl ?>');return false"><i class="fdb_nav_ico">♥</i> <?php echo $__Context->lang->cmd_vote_down ?></a><?php } ?>
			<?php } ?>
			<?php if((!$__Context->mi->cmt_this_btn && $__Context->is_logged) || $__Context->grant->manager){ ?><a class="comment_<?php echo $__Context->comment->comment_srl ?> m_no" href="#popup_menu_area" onclick="return false"><span class="tx_ico_chk">✔</span><?php echo $__Context->lang->cmd_comment_do ?></a><?php } ?>
			<?php if($__Context->comment->isGranted() || !$__Context->comment->get('member_srl')){ ?>
			<a href="<?php echo getUrl('act','dispBoardModifyComment','comment_srl',$__Context->comment->comment_srl) ?>"><span class="ico_16px write"></span><?php echo $__Context->lang->cmd_modify ?></a>
			<a href="<?php echo getUrl('act','dispBoardDeleteComment','comment_srl',$__Context->comment->comment_srl) ?>"><span class="ico_16px delete"></span><?php echo $__Context->lang->cmd_delete ?></a>
			<?php } ?>
			<?php if($__Context->oDocument->allowComment()){ ?><a class="re_comment" href="<?php echo getUrl('act','dispBoardReplyComment','comment_srl',$__Context->comment->comment_srl) ?>" onclick="<?php if(!$__Context->grant->write_comment || !$__Context->oDocument->isEnableComment()){ ?>alert('<?php echo $__Context->lang->msg_not_permitted ?>')<?php }else{ ?>reComment(<?php echo $__Context->comment->get('document_srl') ?>,<?php echo $__Context->comment->get('comment_srl') ?>,'<?php echo getUrl('act','dispBoardReplyComment','comment_srl',$__Context->comment->comment_srl) ?>');<?php } ?>return false"><span class="ico_16px reply"></span><?php echo $__Context->lang->cmd_reply ?></a><?php } ?>
			...
			<?php if($__Context->mi->cmt_vote!='N' && ($__Context->mi->cmt_vote=='2' || $__Context->comment->get('voted_count')!=0 || $__Context->comment->get('blamed_count')!=0)){ ?><span class="vote ui_font">
				<a class="bd_login" href="#"<?php if($__Context->is_logged){ ?> onclick="doCallModuleAction('comment','procCommentVoteUp','<?php echo $__Context->comment->comment_srl ?>');return false"<?php } ?> title="<?php echo $__Context->lang->cmd_vote ?>"><em><b>♥</b> <?php echo $__Context->comment->get('voted_count')?$__Context->comment->get('voted_count'):0 ?></em></a>
				<?php if(!$__Context->mi->cmt_vote_down){ ?><a class="bd_login" href="#"<?php if($__Context->is_logged){ ?> onclick="doCallModuleAction('comment','procCommentVoteDown','<?php echo $__Context->comment->comment_srl ?>');return false"<?php } ?> title="<?php echo $__Context->lang->cmd_vote_down ?>"><b>♥</b> <?php echo abs($__Context->comment->get('blamed_count')?$__Context->comment->get('blamed_count'):0) ?></a><?php } ?>
			</span><?php } ?>
		</div>
	</li>
	<?php } ?>
...
<div class="bd_pg clear <?php echo $__Context->mi->fdb_hide ?>">
	<a href="<?php echo getUrl($__Context->cpageStr,1) ?>#<?php echo $__Context->oDocument->get('document_srl') ?>_comment" class="direction" title="<?php echo $__Context->lang->first_page ?>">&laquo; <span>First</span></a> 
	<?php while($__Context->page_no = $__Context->oDocument->comment_page_navigation->getNextPage()){ ?>
		<?php if($__Context->cpage == $__Context->page_no){ ?>
			<strong class="this"><?php echo $__Context->page_no ?></strong> 
		<?php }else{ ?>
		...
		<?php } ?>
	<?php } ?>
	<a href="<?php echo getUrl($__Context->cpageStr,$__Context->oDocument->comment_page_navigation->last_page) ?>#<?php echo $__Context->oDocument->get('document_srl') ?>_comment" class="direction" title="<?php echo $__Context->lang->last_page ?>"><span>Last</span> &raquo;</a>
</div>
<?php if(!$__Context->mi->fdb_style){ ?><script type="text/javascript">
jQuery(function($){
	var t = $('#<?php echo $__Context->oDocument->document_srl ?>_comment .bd_pg');
