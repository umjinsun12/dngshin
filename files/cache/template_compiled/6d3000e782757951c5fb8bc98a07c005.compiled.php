<?php if(!defined("__XE__"))exit;?><!--#Meta:modules/cympusadmin/tpl/css/style.css--><?php $__tmp=array('modules/cympusadmin/tpl/css/style.css','','','');Context::loadFile($__tmp);unset($__tmp); ?>
<!--#Meta:modules/cympusadmin/tpl/css/default.css--><?php $__tmp=array('modules/cympusadmin/tpl/css/default.css','','','');Context::loadFile($__tmp);unset($__tmp); ?>
<!--#Meta:modules/cympusadmin/tpl/js/toggle.js--><?php $__tmp=array('modules/cympusadmin/tpl/js/toggle.js','','','');Context::loadFile($__tmp);unset($__tmp); ?>
<!--#Meta:modules/cympusadmin/tpl/js/updatenews.js--><?php $__tmp=array('modules/cympusadmin/tpl/js/updatenews.js','','','');Context::loadFile($__tmp);unset($__tmp); ?>
<?php $__Context->oElearningClass = &getClass('elearning') ?>
<div id="admin_wrap">
	<div class="left_panel">
		<h1><a href="<?php echo getUrl('','module','cympusadmin') ?>">ADMIN</a></h1>
		<div class="date">
		...
		<div class="sub_navi">
			<ul>
				...
				<a<?php if(0){ ?> href="<?php echo getUrl('','module','cympusadmin','act',$__Context->val->action[0]) ?>"<?php } ?> onClick="toggleset('opqrst_sub_<?php echo $__Context->idxno ?>')" <?php if($__Context->val->selected){ ?> class="active"<?php } ?> title="<?php echo strip_tags($__Context->val->description) ?>"><?php if(!$__Context->val->selected){ ?><img src="/modules/cympusadmin/tpl/img/menu_<?php echo $__Context->val->icon ?>.png" class="menu_img"/><?php };
if($__Context->val->selected){ ?><img src="/modules/cympusadmin/tpl/img/menuover_<?php echo $__Context->val->icon ?>.png" class="menuover_img"><?php };
...
$__Context->selected_submenu=$__Context->val;
					...
					<?php if($__Context->val->submenu){ ?><ul id="opqrst_sub" class="opqrst_sub_<?php echo $__Context->idxno - 1 ?>">
						<?php if($__Context->val->submenu&&count($__Context->val->submenu))foreach($__Context->val->submenu as $__Context->key2=>$__Context->val2){ ?><li>
							<a href="<?php echo getUrl('','module','cympusadmin','act',$__Context->val2->action[0]) ?>"<?php if($__Context->val2->selected){ ?> class="active"<?php } ?> title="<?php echo strip_tags($__Context->val2->description) ?>"><?php echo $__Context->val2->title ?></a>
...
if($__Context->oElearningClass){ ?> href="http://www.cympus.com"<?php } ?> title="visiting NURIGO" target="_blank">NURIGO</a></strong> all rights reserved.</span>
		</div>
	</div><!-- admin_footer -->
</div><!-- admin_wrap -->
