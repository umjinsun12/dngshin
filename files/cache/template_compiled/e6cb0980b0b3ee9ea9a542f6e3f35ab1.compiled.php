...
<form action="./" class="x_form-horizontal" method="post"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	...
	<div class="x_control-group">
		...
		<div class="x_controls">
			<input type="number" min="0" id="limit_day" name="limit_day" value="<?php echo $__Context->config->limit_day ?>" /> <?php echo $__Context->lang->unit_day ?>
			<p class="x_help-block"><?php echo $__Context->lang->about_limit_day ?></p>
			<input type="text" name="limit_day_description" value="<?php echo $__Context->config->limit_day_description ?>" placeholder="<?php echo $__Context->lang->limit_day_description ?>" style="width:90%" class="lang_code" />
		</div>
	</div>
	<div class="x_control-group">
		...
		<div class="x_controls">
			...
			<ul class="textList" id="deniedNickNameList" style="margin-left:0">
				<?php if($__Context->deniedNickNames&&count($__Context->deniedNickNames))foreach($__Context->deniedNickNames as $__Context->nicknameInfo){ ?><li id="denied_<?php echo $__Context->nicknameInfo->nick_name ?>"><?php echo $__Context->nicknameInfo->nick_name ?> <button type="button" class="x_icon-remove" onclick="doUpdateDeniedNickName('<?php echo $__Context->nicknameInfo->nick_name ?>','delete','<?php echo $__Context->lang->confirm_delete ?>');return false;"><?php echo $__Context->lang->delete ?></button></li><?php } ?>
			</ul>
...
#smart_content>.tool{clear:none !important}
	...
	<?php $__Context->fixed_public_list = array('nick_name', 'find_account_question', 'password') ?>
					...
					<tr class="sticky">
						...
						<th scope="row">
							<span class="_title" style="display:inline-block;white-space:nowrap;overflow:hidden;width:120px;text-overflow:ellipsis;padding-left:20px"><?php echo $__Context->item->title ?> <a class="x_icon-question-sign" href="./admin/help/index.html#UMAN_login_key" target="_blank"><?php echo $__Context->lang->help ?></a></span>
						</th>
						<td style="text-align:center"><input type="radio" name="identifier" value="<?php echo $__Context->item->name ?>" checked="checked" /></td>
						<td style="text-align:center"><input type="checkbox" name="usable_list[]" value="<?php echo $__Context->item->name ?>" title="<?php echo $__Context->lang->use ?>" checked="checked"  disabled="disabled" /></td>
						<td style="text-align:center"><input type="checkbox" name="is_<?php echo $__Context->item->name ?>_public" value="Y"<?php if($__Context->item->isPublic == 'Y'){ ?> checked="checked"<?php } ?>/></td>
						<td class="nowr">
							<label for="<?php echo $__Context->item->name ?>_re" class="x_inline"><input type="radio" id="<?php echo $__Context->item->name ?>_re" name="<?php echo $__Context->item->name ?>" value="requierd" checked="checked" disabled="disabled" /> <?php echo $__Context->lang->cmd_required ?></label>
							<label for="<?php echo $__Context->item->name ?>_op" class="x_inline"><input type="radio" id="<?php echo $__Context->item->name ?>_op" name="<?php echo $__Context->item->name ?>" value="option" disabled="disabled" /> <?php echo $__Context->lang->cmd_optional ?></label>
						</td>
						<td class="desc">&nbsp;</td>
					...
					<tr<?php if($__Context->item->imageType){ ?> class="_imageType"<?php } ?>>
						...
						<th scope="row">
							<div class="wrap">
								...
								<span class="_title" style="display:inline-block;white-space:nowrap;overflow:hidden;width:120px;text-overflow:ellipsis" title="<?php echo $__Context->item->title ?>"><?php echo $__Context->item->title ?></span>
							</div>
						</th>
						<td style="text-align:center"><?php if($__Context->item->name== 'email_address' || $__Context->item->name=='user_id'){ ?><input type="radio" name="identifier" value="<?php echo $__Context->item->name ?>" /><?php } ?></td>
						<td style="text-align:center"><input type="checkbox" name="usable_list[]" value="<?php echo $__Context->item->name ?>" title="<?php echo $__Context->lang->use ?>"<?php if($__Context->item->mustRequired || $__Context->item->isUse){ ?> checked="checked"<?php };
...
if(in_array($__Context->item->name, $__Context->fixed_public_list) || !$__Context->item->isUse){ ?> disabled="disabled"<?php } ?> /></td>
						<td class="nowr">
							<label for="<?php echo $__Context->item->name ?>_re" class="x_inline"><input type="radio" id="<?php echo $__Context->item->name ?>_re" name="<?php echo $__Context->item->name ?>" value="required"<?php if($__Context->item->mustRequired || $__Context->item->required){ ?> checked="checked"<?php };
if($__Context->item->mustRequired || !$__Context->item->isUse){ ?> disabled="disabled"<?php } ?> /> <?php echo $__Context->lang->cmd_required ?></label>
							<label for="<?php echo $__Context->item->name ?>_op" class="x_inline"><input type="radio" id="<?php echo $__Context->item->name ?>_op" name="<?php echo $__Context->item->name ?>" value="option"<?php if(!$__Context->item->mustRequired && ($__Context->item->isUse && !$__Context->item->required)){ ?> checked="checked"<?php };
if($__Context->item->mustRequired || !$__Context->item->isUse){ ?> disabled="disabled"<?php } ?> /> <?php echo $__Context->lang->cmd_optional ?></label>
							<?php if($__Context->item->imageType){ ?><div class="_subItem"<?php if(!$__Context->item->isUse){ ?> style="display:none"<?php } ?>>
								<label for="<?php echo $__Context->item->name ?>_max_width" class="x_inline"><?php echo $__Context->lang->cmd_image_max_width ?> <input type="number" min="1" name="<?php echo $__Context->item->name ?>_max_width" id="<?php echo $__Context->item->name ?>_max_width" value="<?php echo $__Context->item->max_width ?>" /> px</label>
								/
								<label for="<?php echo $__Context->item->name ?>_max_height" class="x_inline"><?php echo $__Context->lang->cmd_image_max_height ?> <input type="number" min="1" name="<?php echo $__Context->item->name ?>_max_height" id="<?php echo $__Context->item->name ?>_max_height" value="<?php echo $__Context->item->max_height ?>" /> px</label>
							</div><?php } ?>
					...
					<tr>
						...
						<td></td>
						<td style="text-align:center"><input type="checkbox" name="usable_list[]" value="<?php echo $__Context->item->name ?>" title="<?php echo $__Context->lang->use ?>"<?php if($__Context->item->isUse){ ?> checked="checked"<?php } ?> /></td>
						<td style="text-align:center"><input type="checkbox" name="is_<?php echo $__Context->item->name ?>_public" value="Y"<?php if($__Context->item->isPublic == 'Y'){ ?> checked="checked"<?php };
...
if(!$__Context->item->isUse){ ?> disabled="disabled"<?php } ?> /> <?php echo $__Context->lang->cmd_optional ?></label>
						...
						<td id="<?php echo $__Context->item->member_join_form_srl ?>" class="nowr" style="text-align:center"><a href="#userDefine" class="modalAnchor _extendFormEdit"><?php echo $__Context->lang->cmd_edit ?></a> <i>|</i> <a href="#" class="_extendFormDelete"><?php echo $__Context->lang->cmd_delete ?></a></td>
					</tr>
					<?php } ?>
					<?php } ?>
				</tbody>
