...
if (!$__Context->mi->thumb_width) $__Context->mi->thumbnail_width = 640;
	if (!$__Context->mi->thumb_height) $__Context->mi->thumbnail_height = 320;
	if (!$__Context->mi->content_cut_size) $__Context->mi->content_cut_size = 100;
 ?>
<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/m.skins/flatBoard','_list_header.html') ?>
...
if($__Context->layout_info->mCol == 'black'){ ?><img src="/modules/board/m.skins/flatBoard/images/biComment2@2x.png" alt="comment" width="14px" height="12px" class="comment-image" /><?php }else{ ?><img src="/modules/board/m.skins/flatBoard/images/biComment@2x.png" alt="comment" width="14px" height="12px" class="comment-image" /><?php } ?><a href="<?php echo $__Context->document->getPermanentUrl() ?>#comment" class="reply m-list-reply"><?php echo $__Context->document->getCommentCount() ?></a><?php } ?>
		</li>
		<?php } ?>
	...
	<ul class="list">
		...
		<li>
			<div class="bubble clearfix">
				...
				<div class="left-side">
				<div class="title el"><?php if($__Context->module_info->use_category=='Y' && $__Context->mi->show_category != 'N'){ ?><span class="category"><?php echo $__Context->category_list[$__Context->document->get('category_srl')]->title ?></span><?php } ?> <?php echo $__Context->document->getTitle();
...
<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/m.skins/flatBoard','_list_footer.html') ?>