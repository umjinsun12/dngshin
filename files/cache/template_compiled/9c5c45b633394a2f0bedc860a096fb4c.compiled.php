<?php if(!defined("__XE__"))exit;?><!--#Meta:modules/editor/tpl/js/editor_common.min.js--><?php $__tmp=array('modules/editor/tpl/js/editor_common.min.js','','','');Context::loadFile($__tmp);unset($__tmp); ?>
<?php if(!Mobile::isFromMobilePhone()){ ?><!--#Meta:modules/board/skins/sketchbook5_youtube/js/xe_textarea.min.js--><?php $__tmp=array('modules/board/skins/sketchbook5_youtube/js/xe_textarea.min.js','','','');Context::loadFile($__tmp);unset($__tmp);
} ?>
<?php if(Mobile::isFromMobilePhone()){ ?><!--#Meta:modules/board/skins/sketchbook5_youtube/js/xe_textarea_m.js--><?php $__tmp=array('modules/board/skins/sketchbook5_youtube/js/xe_textarea_m.js','','','');Context::loadFile($__tmp);unset($__tmp);
} ?>
<?php if(Mobile::isMobileCheckByAgent()){ ?>
<?php  $__Context->mi->cmt_wrt = 'simple' ?>
<?php } ?>
<div class="cmt_editor"<?php if($__Context->mi->cmt_wrt_position=='cmt_wrt_btm' && $__Context->oDocument->getCommentcount()){ ?> style="margin-top:30px"<?php } ?>>
...
<form action="./" method="post" onsubmit="return procFilter(this, insert_comment)" class="bd_wrt cmt_wrt clear"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	...
	<?php if($__Context->mi->cmt_wrt=='editor'){;
echo $__Context->oDocument->getCommentEditor();
} ?>
	<?php if($__Context->mi->cmt_wrt=='simple'){ ?><div class="simple_wrt">
		...
		<div class="text">
			...
			<textarea id="editor_<?php echo $__Context->oDocument->document_srl ?>" cols="50" rows="3"></textarea>
		</div>
		<input type="submit" value="<?php echo $__Context->lang->cmd_submit ?>" class="btn" />
	</div><?php } ?>
	<div class="edit_opt clear"<?php if($__Context->mi->cmt_wrt!='editor'){ ?> style="display:none"<?php } ?>>
		...
		<?php } ?>
		<?php if($__Context->mi->cmt_wrt=='editor' || Mobile::isMobileCheckByAgent()){ ?><button type="submit" class="btn fr"><?php echo $__Context->lang->cmd_registration ?></button><?php } ?>
		<div class="opt_chk">
			<?php if($__Context->is_logged){ ?>
...
<?php if($__Context->rd_idx==0){ ?><div id="re_cmt">
	<label for="editor_2" class="cmt_editor_tl fl"><i class="ico_16px re"></i><strong><?php echo $__Context->lang->write_comment ?></strong></label>
	<div class="editor_select fr">
		<a class="wysiwyg m_no" href="#"><b class="tx_ico_circ bg_color"><span class="ie8_only color">●</span><b>?</b></b> <?php echo $__Context->lang->use_wysiwyg ?></a>
		<a class="close" href="#" onclick="jQuery('#re_cmt').fadeOut().parent().find('.re_comment').focus();return false"><i class="ui-icon ui-icon-closethick"></i><?php echo $__Context->lang->cmd_close ?></a>
	</div>
	<form action="./" method="post" onsubmit="return procFilter(this,insert_comment)" class="bd_wrt clear"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
		...
		<div class="edit_opt">
			...
			<?php } ?>
			<button type="submit" class="btn_img fr"><span class="ico_16px write"></span> <?php echo $__Context->lang->cmd_reply ?></button>
		</div>
		<span class="opt_chk">
			<?php if($__Context->is_logged){ ?>
...
<div class="bd_wrt">
	<div class="simple_wrt">
		...
		<div class="text">
			<?php if(!$__Context->is_logged){ ?><a class="cmt_disable bd_login" href="#"><p><?php echo $__Context->lang->write_comment ?> <?php echo $__Context->lang->msg_not_permitted ?> <?php echo $__Context->lang->bd_login ?></p></a><?php } ?>
			<?php if($__Context->is_logged){ ?><div class="cmt_disable bd_login"><p><?php echo $__Context->lang->write_comment ?> <?php echo $__Context->lang->msg_not_permitted ?></p></div><?php } ?>
		</div>
		<input type="submit" value="<?php echo $__Context->lang->cmd_submit ?>" disabled="disabled" class="btn" />
	</div>
</div>
<?php } ?>
</div>