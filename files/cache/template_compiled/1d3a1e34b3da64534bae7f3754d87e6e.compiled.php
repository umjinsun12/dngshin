...
<?php } ?>
#bd_lst .cate span,#bd_lst .author span,#bd_lst .last_post small{max-width:<?php echo $__Context->mi->normal_lst_tdW ?>px}
</style>
<?php if(!$__Context->document_list && !$__Context->notice_list){ ?><p class="no_doc"><?php echo $__Context->lang->no_documents ?></p><?php } ?>
<?php if($__Context->document_list || $__Context->notice_list){ ?><table id="bd_lst" class="bd_lst bd_tb">
...
if($__Context->document_srl!=$__Context->document->document_srl){ ?><strong><?php echo $__Context->lang->notice ?></strong><?php } ?>
			...
			<td class="title">
				...
				<?php if($__Context->document->getCommentCount() && !$__Context->mi->link_board){ ?><a href="<?php echo getUrl('document_srl', $__Context->document->document_srl) ?>#comment" class="replyNum" title="<?php echo $__Context->lang->comment ?>"><?php echo $__Context->document->getCommentCount() ?></a><?php } ?>
				<?php if($__Context->document->getTrackbackCount() && !$__Context->mi->link_board){ ?><a href="<?php echo getUrl('document_srl', $__Context->document->document_srl) ?>#trackback" class="trackbackNum" title="<?php echo $__Context->lang->trackback ?>"><?php echo $__Context->document->getTrackbackCount() ?></a><?php } ?>
			...
			<?php if($__Context->val->type=='last_post' && $__Context->val->idx==-1){ ?><td class="time last_post m_no">
				<?php if((int)($__Context->document->get('comment_count'))>0){ ?>
					<a href="<?php echo $__Context->document->getPermanentUrl() ?>#comment" title="Last Reply"><?php echo zdate($__Context->document->get('last_update'),'Y.m.d') ?></a>
					<?php if($__Context->document->get('last_updater')){ ?><small>(by <?php echo htmlspecialchars($__Context->document->get('last_updater')) ?>)</small><?php } ?>
				<?php } ?>
			...
			<?php if($__Context->val->idx!=-1){ ?><td<?php if($__Context->val->eid!='link_url'){ ?> class="m_no"<?php } ?>><?php echo $__Context->document->getExtraValueHTML($__Context->val->idx) ?></td><?php } ?>
			<?php } ?>
...
echo $__Context->no;
			...
			<?php if($__Context->val->type=='title' && $__Context->val->idx==-1){ ?>
			<?php if(!$__Context->mi->show_cate && $__Context->module_info->use_category=='Y'){ ?><td class="cate"><span<?php if($__Context->category_list[$__Context->document->get('category_srl')]->color!='transparent'){ ?> style="color:<?php echo $__Context->category_list[$__Context->document->get('category_srl')]->color ?>"<?php } ?>><?php echo $__Context->category_list[$__Context->document->get('category_srl')]->title ?></span></td><?php } ?>
			<td class="title">
				...
				<?php if($__Context->document->getCommentCount()){ ?><a href="<?php echo getUrl('document_srl', $__Context->document->document_srl) ?>#comment" class="replyNum" title="<?php echo $__Context->lang->comment ?>"><?php echo $__Context->document->getCommentCount() ?></a><?php } ?>
				<?php if($__Context->document->getTrackbackCount()){ ?><a href="<?php echo getUrl('document_srl', $__Context->document->document_srl) ?>#trackback" class="trackbackNum" title="<?php echo $__Context->lang->trackback ?>"><?php echo $__Context->document->getTrackbackCount() ?></a><?php } ?>
				<?php } ?>
...
if($__Context->document->thumbnailExists()){ ?><i class="attached_image" title="Image"></i><?php } ?></span>
			...
			<?php if($__Context->val->type=='last_post' && $__Context->val->idx==-1){ ?><td class="time last_post m_no">
				<?php if((int)($__Context->document->get('comment_count'))>0){ ?>
					<a href="<?php echo $__Context->document->getPermanentUrl() ?>#comment" title="<?php echo getTimeGap($__Context->document->get('last_update'), "H:i") ?>"><?php echo zdate($__Context->document->get('last_update'),'Y.m.d') ?></a>
					<?php if($__Context->document->get('last_updater')){ ?><small>(by <?php echo htmlspecialchars($__Context->document->get('last_updater')) ?>)</small><?php } ?>
				<?php } ?>
				<?php if((int)($__Context->document->get('comment_count'))==0){ ?>&nbsp;<?php } ?>
			</td><?php } ?>
