		...
		<tbody>
			<?php if($__Context->comment_list&&count($__Context->comment_list))foreach($__Context->comment_list as $__Context->no=>$__Context->val){ ?><tr>
				...
				<td>
					...
					<?php } ?>
					<a href="<?php echo getUrl('','document_srl',$__Context->val->document_srl) ?>#comment_<?php echo $__Context->val->comment_srl ?>" target="_blank"><?php if(strlen($__Context->comment)){;
echo $__Context->comment;
}else{ ?><em><?php echo $__Context->lang->no_text_comment ?></em><?php } ?></a>
				</td>
