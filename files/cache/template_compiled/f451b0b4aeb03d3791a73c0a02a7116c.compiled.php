...
<?php Context::addJsFile("modules/member/ruleset/insertAdminMember.xml", FALSE, "", 0, "body", TRUE, "") ?><form action="./" class="x_form-horizontal"  method="post" enctype="multipart/form-data"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="insertAdminMember" />
	...
	<?php if($__Context->member_srl){ ?><div class="x_control-group">
		<label class="x_control-label" for="identifierForm"><em style="color:red">*</em> <?php echo $__Context->identifierForm->title ?> <a class="x_icon-question-sign" href="./admin/help/index.html#UMAN_login_key" target="_blank"><?php echo $__Context->lang->help ?></a></label>
	...
	<?php if($__Context->member_srl){ ?><div class="x_control-group">
		...
		<div class="x_controls">
			<input type="hidden" name="password" value="<?php echo $__Context->member_info->password ?>" />
...
.xpress-editor>#smart_content>.tool{clear:none}
	...
	<?php if($__Context->member_srl){ ?><div class="x_control-group">
		...
		<div class="x_controls">
			...
			<input type="text" readonly placeholder="YYYY-MM-DD" class="inputDate" id="until" value="<?php echo zdate($__Context->member_info->limit_date,'Y-m-d',false) ?>" />
	...
	<div class="x_control-group">
		...
		<div class="x_controls">
			<?php if($__Context->group_list&&count($__Context->group_list))foreach($__Context->group_list as $__Context->key=>$__Context->val){ ?><label for="group_<?php echo $__Context->key ?>" class="x_inline">
