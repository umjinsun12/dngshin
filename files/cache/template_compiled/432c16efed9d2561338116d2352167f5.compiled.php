...
<!--#Meta:modules/nproduct/skins/default/css/_style.list.css--><?php $__tmp=array('modules/nproduct/skins/default/css/_style.list.css','','','');Context::loadFile($__tmp);unset($__tmp); ?>
...
<table class="item-table" width="100%">
	...
	<tbody>
		<?php if($__Context->list&&count($__Context->list))foreach($__Context->list as $__Context->no=>$__Context->val){ ?><tr id="item_<?php echo $__Context->val->item_srl ?>">
			...
			<?php if($__Context->val->discount_amount && $__Context->k == 'amount'){ ?><td><span style="text-decoration:line-through;"><?php echo $__Context->val->printPrice() ?></span><br /><span class="font_size12"><strong><?php echo $__Context->val->printDiscountedPrice() ?></strong></span></td><?php } ?>
			...
			<?php if($__Context->k == 'download_count'){ ?><td><?php echo $__Context->val->download_count ?></td><?php } ?>
