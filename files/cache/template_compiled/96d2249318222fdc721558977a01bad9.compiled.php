...
sejin7940_editor_fileuploade  
sejin7940  (  http://sejin7940.co.kr  /  sejin7940@nate.com )
ver 1.0  ( 150131 )
일반 에디터는  $hide_icon='N',  $hide_content='N',  $hide_file='N' 과 동일함
파일 전용업로드 에디터로 설정하려면  $hide_icon='Y',  $hide_content='Y',  $hide_file='N'  로 설정하면 됨
파일첨부는 가능하나 위지윅에디터 기능이 안 뜨게 하려면  $hide_icon='Y',  $hide_content='N',  $hide_file='N'  로 설정하면 됨
그냥 내용입력창만 뜨게 하려면,  $hide_icon='Y', $hide_content='N',  $hide_file='Y'  로 설정하면 됨
썸네일 강제지정 기능 사용원하는 경우, 첨부된 sejin7940_thumb.txt 파일 참조해서 Core 소스 수정해야함
포토에디터 노출 기능 사용은,  포토에디터를 다운 받아서 (http://www.xpressengine.com/index.php?mid=download&package_srl=18324280)  
    modules/editor/components/  안에 업로드하고 관리자페이지->모듈->에디터->컴퍼넌트 쪽에 체크해줘야함
-->
<!-- 에디터 설정 -->
<?php 
$__Context->hide_icon = "Y";  // 웹에디터의 아이콘 숨김 여부  -  Y(숨김) ,N(보임)
$__Context->hide_content = "Y";  // 웹에디터의 내용입력창 숨김 여부  -  Y(숨김) ,N(보임) 
$__Context->default_content = ".";  // 내용숨김시 들어갈 기본 내용값
$__Context->hide_file = "N";  // 파일업로드창 숨김 여부 - Y(숨김) ,N(보임)
$__Context->select_thumbnail ="N";   // 썸네일 강제지정 기능 사용여부 - Y (사용),  N (사용 안 함)  =>  사용 원할 경우 첨부된 sejin7940_thumb.txt 파일 참조해서 Core 소스 수정해야함
$__Context->display_photoeditor ="N";    // 포토에디터 확장컴퍼넌트를 사용하는 경우, 손쉽게 사용가능하도록 버튼 외부 노출 여부  ( Y : 사용, N : 사용 안 함 )
 ?>
<style>
.fileupload_box { }   /* 파일업로드 박스에 css 적용위해서  ->  예)  padding:7px 10px 0px 10px; background:#f7f7f7; border:1px solid #ccc; margin-top:7px;   */
</style>
<?php if($__Context->hide_content=='Y'){;
$__Context->hide_icon="Y";
} ?>
<?php if($__Context->oDocument->getContentText()){;
$__Context->default_content = $__Context->oDocument->getContentText();
} ?>
<!--#Meta:modules/editor/skins/sejin7940_editor_fileupload/css/default.css--><?php $__tmp=array('modules/editor/skins/sejin7940_editor_fileupload/css/default.css','','','');Context::loadFile($__tmp);unset($__tmp); ?>
...
<?php if($__Context->colorset == "black" || $__Context->colorset == "black_texteditor" || $__Context->colorset == "black_text_nohtml" || $__Context->colorset == "black_text_usehtml"){ ?>
    <!--#Meta:modules/editor/skins/sejin7940_editor_fileupload/css/black.css--><?php $__tmp=array('modules/editor/skins/sejin7940_editor_fileupload/css/black.css','','','');Context::loadFile($__tmp);unset($__tmp); ?>
...
<?php if($__Context->colorset == "white_texteditor" || $__Context->colorset == "black_texteditor" || $__Context->colorset == "white_text_nohtml" || $__Context->colorset == "black_text_nohtml" || $__Context->colorset == "white_text_usehtml" || $__Context->colorset == "black_text_usehtml"){ ?>
	<?php if(__DEBUG__){ ?><!--#Meta:modules/editor/skins/sejin7940_editor_fileupload/js/xe_textarea.js--><?php $__tmp=array('modules/editor/skins/sejin7940_editor_fileupload/js/xe_textarea.js','','','');Context::loadFile($__tmp);unset($__tmp);
} ?>
	<?php if(!__DEBUG__){ ?><!--#Meta:modules/editor/skins/sejin7940_editor_fileupload/js/xe_textarea.min.js--><?php $__tmp=array('modules/editor/skins/sejin7940_editor_fileupload/js/xe_textarea.min.js','','','');Context::loadFile($__tmp);unset($__tmp);
} ?>
...
<?php }else{ ?>
	...
	<?php if(__DEBUG__){ ?><!--#Meta:modules/editor/skins/sejin7940_editor_fileupload/js/Xpress_Editor.js--><?php $__tmp=array('modules/editor/skins/sejin7940_editor_fileupload/js/Xpress_Editor.js','','','');Context::loadFile($__tmp);unset($__tmp);
} ?>
	<?php if(__DEBUG__){ ?><!--#Meta:modules/editor/skins/sejin7940_editor_fileupload/js/xe_interface.js--><?php $__tmp=array('modules/editor/skins/sejin7940_editor_fileupload/js/xe_interface.js','','','');Context::loadFile($__tmp);unset($__tmp);
} ?>
	<?php if(!__DEBUG__){ ?><!--#Meta:modules/editor/skins/sejin7940_editor_fileupload/js/xpresseditor.min.js--><?php $__tmp=array('modules/editor/skins/sejin7940_editor_fileupload/js/xpresseditor.min.js','','','');Context::loadFile($__tmp);unset($__tmp);
} ?>
    <!-- 자동저장용 폼 -->
    ...
    <?php } ?>
	<!-- 에디터 -->
    <div class="xpress-editor <?php echo $__Context->editor_class ?> fileupload_box" style="<?php if($__Context->module_info->skin=='default'){ ?>background:#fff; padding-top:10px; <?php } ?> " >  
        <div id="smart_content" <?php if($__Context->hide_content=='Y'){ ?>style="display:none;"<?php } ?>> <a href="#xe-editor-container-<?php echo $__Context->editor_sequence ?>" class="skip">&raquo; <?php echo $__Context->lang->edit->jumptoedit ?></a>
        <?php if($__Context->enable_default_component||$__Context->enable_component||$__Context->html_mode){ ?>
        <!-- 편집 컴포넌트 -->
        <div class="tool off" <?php if($__Context->hide_icon=='Y'){ ?>style="display:none"<?php } ?>>
		...
		<?php if($__Context->allow_fileupload){ ?><div id="fileUploader_<?php echo $__Context->editor_sequence ?>" class="fileUploader"><!--File upload zone-->
            <div class="preview <?php echo $__Context->btn_class ?>" id="preview_uploaded_<?php echo $__Context->editor_sequence ?>" style="background:#fff;"></div>
            ...
            <div class="fileUploadControl">
				...
				<?php if($__Context->display_photoeditor=='Y'){ ?>
				<span id="component_<?php echo $__Context->editor_sequence ?>_photo_editor" style="cursor:pointer; font-weight:bold; padding-left:10px;">[포토에디터]</span>
				<?php } ?>
            </div>
            <div class="file_attach_info" id="uploader_status_<?php echo $__Context->editor_sequence ?>"><?php echo $__Context->upload_status ?></div>
			<?php if($__Context->select_thumbnail=='Y' && $__Context->module!=='admin'){ ?>
				<?php if($__Context->act=='dispBoardReplyComment' || $__Context->act=='dispBoardModifyComment'){ ?>
				<input type="hidden" name="thumb_image" id="thumb_image" value="<?php echo $__Context->oComment->get('thumb_image') ?>" />
				<?php }else{ ?>
				<input type="hidden" name="thumb_image" id="thumb_image" value="<?php echo $__Context->oDocument->get('thumb_image') ?>" />
				<?php } ?>
				<input type="hidden" name="thumb_sequence" value="" />
				<input type="hidden" name="tmp_thumb" value="" />
				<div style="margin-top:5px;">
					<input type="checkbox" id="isThumbnail" /> <?php echo $__Context->lang->thumb_image_chk ?>
				</div>
			<?php } ?>
        </div><?php } ?>
		<?php } ?>
    ...
    <script type="text/javascript">//<![CDATA[
		...
		<?php if($__Context->hide_content=='Y'){ ?>
			var content_length = document.getElementsByName('content').length;
			for (var c=0;c < content_length ;c++ )	{
				document.getElementsByName('content')[c].value = '<?php echo $__Context->default_content ?>';	
			}
//			document.getElementById('postContent').value = '<?php echo $__Context->default_content ?>';	
		<?php } ?>
    //]]></script>
	<?php if($__Context->select_thumbnail=='Y' && $__Context->module!=='admin'){ ?>
	<script>
	...
	jQuery("#isThumbnail").click(function () {
		var opts = document.getElementById('uploaded_file_list_<?php echo $__Context->editor_sequence ?>').options;
		if(!jQuery(this).attr("checked")) {
			jQuery("#isThumbnail").attr("checked", true);
			return;
		}
		else {
			document.getElementById('thumb_image').value = document.getElementById('uploaded_file_list_<?php echo $__Context->editor_sequence ?>').options[document.getElementById('uploaded_file_list_<?php echo $__Context->editor_sequence ?>').options.selectedIndex].value;
		}
	});
