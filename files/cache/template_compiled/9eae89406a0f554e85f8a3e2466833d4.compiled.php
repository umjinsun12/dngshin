<?php if(!defined("__XE__"))exit;?><footer class="clearfix list-footer">
	<?php if($__Context->grant->manager){ ?><div class="left-part">
		<a href="<?php echo getUrl('act','dispBoardAdminBoardInfo') ?>" class="btAdmin"><?php echo $__Context->lang->cmd_setup ?></a>
	</div><?php } ?>
	<?php if($__Context->grant->write_document){ ?><div class="right-part">
		<a href="<?php echo getUrl('act','dispBoardWrite','document_srl','') ?>" class="btBasic"><?php echo $__Context->lang->cmd_write ?></a>
	</div><?php } ?>
