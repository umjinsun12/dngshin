...
<!--#Meta:modules/board/skins/sketchbook5_youtube/js/editor.js--><?php $__tmp=array('modules/board/skins/sketchbook5_youtube/js/editor.js','body','','');Context::loadFile($__tmp);unset($__tmp); ?>
<?php if(Mobile::isMobileCheckByAgent()){;
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/sketchbook5_youtube','_write_form_mobile.html');
} ?>
<?php if(!Mobile::isMobileCheckByAgent()){ ?><form action="./" method="post" onsubmit="return procFilter(this, window.insert)" class="bd_wrt bd_wrt_main clear"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	<input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" />
	<?php if(!$__Context->oDocument->getContentText() && $__Context->mi->content_default){ ?>
		...
		<tr>
			...
			<td width="100%" style="padding-right:12px">
				<span class="itx_wrp">
					...
					<?php if($__Context->oDocument->getTitleText()){ ?><input type="text" name="title" class="itx" id="postTitle" title="<?php echo $__Context->lang->title ?>" value="<?php echo htmlspecialchars($__Context->oDocument->getTitleText()) ?>" /><?php } ?>
					<?php if(!$__Context->oDocument->getTitleText()){ ?><input type="text" name="title" class="itx" id="postTitle" title="<?php echo $__Context->lang->title ?>" /><?php } ?>
				</span>
			</td>
			<?php if($__Context->is_logged && !$__Context->oDocument->isExists() || $__Context->oDocument->get('status')=='TEMP'){ ?><td class="m_no"><button type="button" class="btn" onclick="doDocumentLoad(this);"><?php echo $__Context->lang->cmd_load ?></button></td><?php } ?>
		</tr>
	</table>
	<?php if(count($__Context->extra_keys)){ ?><table class="exForm bd_tb">
...
<!--#JSPLUGIN:ui.colorpicker--><?php Context::loadJavascriptPlugin('ui.colorpicker'); ?>
			<span class="itx_wrp color_wrp" title="<?php echo $__Context->lang->title_color ?>">
				...
				<input type="text" name="title_color" id="title_color" class="color-indicator"<?php if($__Context->oDocument->get('title_color')!='N'){ ?> value="<?php echo $__Context->oDocument->get('title_color') ?>"<?php } ?> />
			</span>
	...
	<div class="regist">
		<?php if($__Context->is_logged && !$__Context->oDocument->isExists() || $__Context->oDocument->get('status')=='TEMP'){ ?><button type="button" onclick="doDocumentSave(this);" class="btn temp"><?php echo $__Context->lang->cmd_temp_save ?></button><?php } ?>
		<input type="button" value="<?php echo $__Context->lang->cmd_preview ?>" onclick="doDocumentPreview(this);return false" class="btn m_no" />
		<input type="submit" value="<?php echo $__Context->lang->cmd_registration ?>" class="btn blue" />
		<button type="button" onclick="history.back()" class="btn cancle"><?php echo $__Context->lang->cmd_back ?></button>
	</div>
</form><?php } ?>
</div>