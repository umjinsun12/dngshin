<?php
    ...
    class store_reviewItem extends Object {
        ...
        function store_reviewItem($review_srl = 0, $columnList = array()) {
            $this->review_srl = $review_srl;
			$this->columnList = $columnList;
            $this->_loadFromDB();
        }

        function setReview($review_srl) {
            $this->review_srl = $review_srl;
            $this->_loadFromDB();
        }

        function _loadFromDB() {
            if(!$this->review_srl) return;

            $args->review_srl = $this->review_srl;
            $output = executeQuery('store_review.getReview', $args, $this->columnList);

            $this->setAttribute($output->data);
        }

        function setAttribute($attribute) {
            if(!$attribute->review_srl) {
                $this->review_srl = null;
                return;
            }
            $this->review_srl = $attribute->review_srl;
            $this->adds($attribute);
        ...
        function isExists() {
            return $this->review_srl ? true : false;
        }

        function isGranted() {
            if($_SESSION['own_review'][$this->review_srl]) return true;
        ...
        function setGrant() {
            $_SESSION['own_review'][$this->review_srl] = true;
            $this->is_granted = true;
        }

        function setAccessible() {
            $_SESSION['accessibled_review'][$this->review_srl] = true;
        }
        ...
        function isAccessible() {
            if($_SESSION['accessibled_review'][$this->review_srl]) return true;
        ...
        function notify($type, $content) {
            ...
            // return if the currently logged-in user is an author of the review.
            $logged_info = Context::get('logged_info');
            if($logged_info->member_srl == $this->get('member_srl')) return;
            // get where the review belongs to 
            $oDocumentModel = &getModel('document');
            $oDocument = $oDocumentModel->getDocument($this->get('document_srl'));
            // Variables
            if($type) $title = "[".$type."] ";
            $title .= cut_str(strip_tags($content), 30, '...');
            $content = sprintf('%s<br /><br />from : <a href="%s#review_%s" target="_blank">%s</a>',$content, getFullUrl('','document_srl',$this->get('document_srl')), $this->get('review_srl'),  getFullUrl('','document_srl',$this->get('document_srl')));
        ...
        function getContent($add_popup_menu = true, $add_content_info = true, $add_xe_content_class = true) {
            ...
            // when displaying the review on the pop-up menu
            if($add_popup_menu && Context::get('is_logged') ) {
                $content = sprintf(
                        '%s<div class="review_popup_menu"><a href="#popup_menu_area" class="review_%d" onclick="return false">%s</a></div>',
                        $content,
                        $this->review_srl, Context::getLang('cmd_review_do')
                );
            }
            // if additional information which can access contents is set
            if($add_content_info) {
                $content = sprintf(
                        '<!--BeforeComment(%d,%d)--><div class="review_%d_%d xe_content">%s</div><!--AfterComment(%d,%d)-->',
                        $this->review_srl, $this->get('member_srl'),
                        $this->review_srl, $this->get('member_srl'),
                        $content,
                        $this->review_srl, $this->get('member_srl')
                );
        ...
        function getPermanentUrl() {
            return getFullUrl('','document_srl',$this->get('document_srl')).'#review_'.$this->get('review_srl');
        }
        ...
        function thumbnailExists($width = 80, $height = 0, $type = '') {
            if(!$this->review_srl) return false;
        ...
        function getThumbnail($width = 80, $height = 0, $thumbnail_type = '') {
            ...
            $thumbnail_path = sprintf('files/cache/thumbnails/%s',getNumberingPath($this->review_srl, 3));
            ...
            if(!$source_file) {
                ...
                for($i=0;$i<$cnt;$i++) {
                    ...
                    if(preg_match('/\/(common|modules|widgets|addons|layouts)\//i', $target_src)) continue;
                        ...
                        $tmp_file = sprintf('./files/cache/tmp/%d', md5(rand(111111,999999).$this->review_srl));
        ...
        function isCarted() {
            return $_SESSION['review_management'][$this->review_srl];
        }
    }
?>
