...
class store_reviewModel extends store_review {
	...
	function getReview($review_srl=0, $is_admin = false, $columnList = array()) {
		debugPrint('review_srl : '. $review_srl);
	    $oReview = new store_reviewItem($review_srl, $columnList);
	    if($is_admin) $oReview->setGrant();
	...
	*/
			...
			debugPrint('getReviewPageList : ' . serialize($output));
	...
	function getTotalReviewInfo($item_srl)
	{
		...
		$review->sum = 0;
		$review->count = 0;
		if(!$output->toBool()) return $review;
		if(!$output->data) return $review;
		$review = $output->data;
		$review->average = 0;
		if($review->count) $review->average = round($review->sum / $review->count);
		return $review;
	}
}
?>
